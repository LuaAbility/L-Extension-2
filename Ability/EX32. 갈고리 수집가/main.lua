...
function Init(abilityData)
	plugin.registerEvent(abilityData, "EX032-collectQuestion", "AsyncPlayerChatEvent", 0)
	plugin.registerEvent(abilityData, "EX032-useAbility", "PlayerInteractEvent", 0)
end

function onEvent(funcTable)
	if funcTable[1] == "EX032-collectQuestion" then collectQuestion(funcTable[3], funcTable[2], funcTable[4], funcTable[1]) end
	if funcTable[1] == "EX032-useAbility" then useAbility(funcTable[3], funcTable[2], funcTable[4], funcTable[1]) end
end

function onTimer(player, ability)
	local count = player:getVariable("EX032-currentQuestion")
	if count == nil then  
		player:setVariable("EX032-currentQuestion", 0)
		player:setVariable("EX032-timerTime", 0)
		count = 0
	end
	
	if player:getVariable("EX032-timerTime") > 200 and player:getVariable("EX032-currentQuestion") < 10 then
		player:setVariable("EX032-currentQuestion", count + 1)
	...
	player:setVariable("EX032-timerTime", player:getVariable("EX032-timerTime") + 1)
	
	game.sendActionBarMessage(player:getPlayer(), "EX032", "§a수집한 갈고리 §6: §b" .. count .. "개")
end

function Reset(player, ability)
	game.sendActionBarMessageToAll("EX032", "")
end

function collectQuestion(LAPlayer, event, ability, id)
	if event:getPlayer() ~= LAPlayer:getPlayer() then
		local count = LAPlayer:getVariable("EX032-currentQuestion")
		...
		if lap and lap.isSurvive and event:getMessage() ~= nil and string.find(event:getMessage(), "?") and string.find(event:getMessage(), "?") > 0 and count < 10 then
			LAPlayer:setVariable("EX032-currentQuestion", count + 1)
			...
			LAPlayer:setVariable("EX032-timerTime", 0)
		end
	end
end
		...
		if event:getItem() ~= nil then
			if game.isAbilityItem(event:getItem(), "IRON_INGOT") then
				if game.checkCooldown(LAPlayer, game.getPlayer(event:getPlayer()), ability, id) then
					local question = LAPlayer:getVariable("EX032-currentQuestion")
					if question > 0 then
						LAPlayer:setVariable("EX032-currentQuestion", question - 1)
		...
		if event:getItem() ~= nil then
			if game.isAbilityItem(event:getItem(), "IRON_INGOT") then
				if game.checkCooldown(LAPlayer, game.getPlayer(event:getPlayer()), ability, id) then
					local question = LAPlayer:getVariable("EX032-currentQuestion")
					if question > 0 then
						LAPlayer:setVariable("EX032-currentQuestion", 0)
						local players = util.getTableFromList(game.getTeamManager():getOpponentTeam(LAPlayer, false))
						for i = 1, #players do
							if players[i]:getPlayer() ~= event:getPlayer() then
								if event:getPlayer():getWorld():getEnvironment() == players[i]:getPlayer():getWorld():getEnvironment() and
